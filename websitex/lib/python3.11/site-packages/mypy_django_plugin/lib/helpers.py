...
def get_django_metadata_bases(
    model_info: TypeInfo, key: Literal["baseform_bases"]
) -> dict[str, int]:
...
def get_or_create_annotated_type(
    api: SemanticAnalyzer, model_info: TypeInfo, annotations_info: TypeInfo
) -> TypeInfo:
    ...
    annotated_model = lookup_fully_qualified_typeinfo(
        api, model_info.fullname + "@AnnotatedWith"
    )
    if annotated_model is None:
        ...
        annotated_model = add_new_class_for_module(
            model_module,
            name=annotated_model_name,
            bases=[model_type, annotations_type],
        )
...
class DjangoModel(NamedTuple):
    ...
    def from_model_type(
        ...
        model_cls = (
            django_context.get_model_class_by_fullname(
                model_info.bases[0].type.fullname
            )
            if is_annotated
...
def get_model_info_from_qs_ctx(
    ...
    if not (
        ...
        and (model_type := extract_model_type_from_queryset(ctx.type))
    ):
        return None
...
def get_call_argument_by_name(
    ctx: FunctionContext | MethodContext, name: str
) -> Expression | None:
    """
    ...
    for kinds, argnames, args in zip(
        ctx.arg_kinds, ctx.arg_names, ctx.args, strict=False
    ):
...
def get_bool_call_argument_by_name(
    ctx: FunctionContext | MethodContext, name: str, *, default: bool
) -> bool:
    """
...
def is_optional(typ: MypyType) -> bool:
    ...
    return isinstance(typ, UnionType) and any(
        isinstance(get_proper_type(item), NoneTyp) for item in typ.items
    )
...
def get_literal_str_type(typ: Type) -> str | None:
    ...
    typ = get_proper_type(typ)
    if (
        isinstance(typ, LiteralType) and isinstance((literal_value := typ.value), str)
    ) or (
...
def get_field_type_args(field_type: Instance) -> FieldTypeArgs | None:
    ...
    base_field_info = next(
        (
            base
            for base in field_type.type.mro
    ...
    return FieldTypeArgs(
        set=get_proper_type(mapped.args[0]), get=get_proper_type(mapped.args[1])
    )
...
def make_oneoff_named_tuple(
    ...
    extra_bases: list[Instance] | None = None,
) -> TupleType:
...
def extend_oneoff_named_tuple(
    ...
    name: str,
    original: TupleType,
    extra_fields: Mapping[str, MypyType],
) -> TupleType:
    existing_fields = {
...
def resolve_string_attribute_value(
    ...
    if (
        isinstance(attr_expr, NameExpr)
        and isinstance(attr_expr.node, Var)
        and attr_expr.node.type is not None
    ):
...
def get_typechecker_api(
    ctx: AttributeContext | MethodContext | FunctionContext,
) -> TypeChecker:
    ...
    if (
        ...
        for stmt in meta.node.defn.defs.body:
            if (
                ...
                is_literal_true = (
                    isinstance(stmt_type, LiteralType) and stmt_type.value is True
                )
...
def merge_extra_attrs(
    ...
    *,
    ...
    if base_extra_attrs:
        return ExtraAttrs(
            attrs={**base_extra_attrs.attrs, **new_attrs}
            if new_attrs is not None
            else base_extra_attrs.attrs.copy(),
            immutable=(
