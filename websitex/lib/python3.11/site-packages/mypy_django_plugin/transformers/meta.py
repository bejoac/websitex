...
from mypy_django_plugin.django.context import (
    DjangoContext,
    get_field_type_from_model_type_info,
)
...
def return_proper_field_type_from_get_field(
    ...
    if not (
        ...
        and (field_name_expr := helpers.get_call_argument_by_name(ctx, "field_name"))
        is not None
        and (
    ...
    if (
        django_model := DjangoModel.from_model_type(model_type, django_context)
    ) is None:
        return ctx.default_return_type

    try:
        field = django_model.cls._meta.get_field(field_name)
        if field_info := helpers.lookup_class_typeinfo(
