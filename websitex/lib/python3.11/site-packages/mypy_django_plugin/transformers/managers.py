...
def _process_dynamic_method(
    ...
    *,
    ...
    manager_model = get_proper_type(
        find_member("model", manager_instance, manager_instance)
    )
    ...
    if _has_compatible_type_vars(base_that_has_method):
        ...
        if (
            ...
            ret_type = _replace_type_var(
                ret_type, base_that_has_method.defn.type_vars[0].fullname, model_arg
            )
            args_types = [
                _replace_type_var(
                    arg_type, base_that_has_method.defn.type_vars[0].fullname, model_arg
                )
    ...
    if base_that_has_method.self_type:
        ...
        variables = tuple(
            v for v in variables if v.id != base_that_has_method.self_type.id
        )
...
def _replace_type_var(
    ...
    if isinstance(ret_type, Instance):
        ...
        return ret_type.copy_modified(
            args=tuple(
                _replace_type_var(item, to_replace, replace_by)
                for item in ret_type.args
            )
        )
...
def resolve_manager_method(ctx: AttributeContext) -> MypyType:
    ...
    if isinstance(ctx.type, UnionType) and all(
...
def reparametrize_generic_class(ctx: ClassDefContext, base_class_fullname: str) -> None:
    ...
    parent_class = next(
        (
            ...
            if base.type.has_base(base_class_fullname)
        ),
        None,
    )
    ...
    if (
        not isinstance(model_param, AnyType)
        or model_param.type_of_any is not TypeOfAny.from_omitted_generics
    ):
        return
