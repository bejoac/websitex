...
def get_actual_types(
    ctx: MethodContext | FunctionContext, expected_keys: list[str]
) -> list[tuple[str, MypyType]]:
    actual_types = []
    # positionals
    for pos, (actual_name, actual_type) in enumerate(
        zip(ctx.arg_names[0], ctx.arg_types[0], strict=False)
    ):
    ...
    if len(ctx.callee_arg_names) > 1:
        for actual_name, actual_type in zip(
            ctx.arg_names[1], ctx.arg_types[1], strict=False
        ):
            if actual_name is None:
                # unpacked dict as kwargs is not supported
                continue
...
def typecheck_model_method(
    ...
    for actual_name, actual_type in get_actual_types(ctx, expected_keys):
        if actual_name not in expected_types:
            ...
            min_arg_count -= (
                1  # To avoid double error (Unexpected attribute + too many arguments)
            )
            continue
    ...
    if min_arg_count > len(expected_keys):
        ctx.api.fail(
            f'Too many arguments for "{model_cls.__name__}"', ctx.context, code=CALL_ARG
        )


def typecheck_model_init(
    ...
    if (
        ...
        and (
            model_cls := django_context.get_model_class_by_fullname(
                default_return_type.type.fullname
            )
        )
        is not None
    ):
...
def typecheck_model_acreate(
    ...
    if (
        ...
        and isinstance(
            (model := get_proper_type(default_return_type.args[-1])), Instance
        )
        and (
