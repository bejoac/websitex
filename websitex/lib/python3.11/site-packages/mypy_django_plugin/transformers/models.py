...
class AddAnnotateUtilities(ModelClassInitializer):
    ...
    @override
    def run(self) -> None:
        annotations = self.lookup_typeinfo_or_incomplete_defn_error(
            "django_stubs_ext.Annotations"
        )
        annotated_model = helpers.get_or_create_annotated_type(
            self.api, self.model_classdef.info, annotations
        )
        if self.is_model_abstract:
            ...
            # back
            exception_bases = {
                model_exc_name: self.lookup_typeinfo_or_incomplete_defn_error(
                    base_exc_name
                )
...
class InjectAnyAsBaseForNestedMeta(ModelClassInitializer):
    ...
    def run(self) -> None:
        ...
        if typed_model_meta_info and not meta_node.has_base(
            fullnames.TYPED_MODEL_META_FULLNAME
        ):
