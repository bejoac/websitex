...
def get_field_type_from_lookup(
    ...
    *,
    ...
    try:
        lookup_field, model_cls = django_context.resolve_lookup_into_field(
            model_cls, lookup
        )
...
def extract_proper_type_queryset_values_list(
    ...
    if flat and named:
        ...
        return reparametrize_queryset(
            default_return_type, [django_model.typ, AnyType(TypeOfAny.from_error)]
        )

    annotation_types = (
        _get_annotation_field_types(django_model.typ)
    ...
    if not named and (
        ...
        ret.extra_attrs = helpers.merge_extra_attrs(
            ret.extra_attrs, new_immutable=set(field_lookups)
        )
    return ret
...
def _resolve_output_field_type(expr_type: MypyType) -> MypyType | None:
    ...
    for arg in proper.args:
        ...
        if isinstance(arg_proper, Instance) and arg_proper.type.has_base(
    ...
    if isinstance(field_type, Instance):
        result = helpers.get_private_descriptor_type(
            field_type.type, "_pyi_private_get_type", is_nullable=False
        )
...
def _extract_model_type_var_upper_bound(ctx: MethodContext) -> Instance | None:
    ...
    for base_type in [ctx.type, *ctx.type.type.bases]:
        if not len(base_type.args) or base_type.type.has_base(
...
def extract_proper_type_queryset_annotate(
        ...
        if upper_bound is not None:
            ...
            if isinstance(default_return_type, Instance):
                ...
                if expression_types:
                    ...
                    upper_annotated = get_annotated_type(
                        api, upper_bound, fields_dict=fields_dict
                    )
                    return reparametrize_queryset(
                        default_return_type, [upper_annotated, upper_annotated]
                    )
    ...
    if all_fields:
        ...
        annotated_type = get_annotated_type(
            api, django_model.typ, fields_dict=fields_dict
        )
    ...
    if len(default_return_type.args) > 1:
        ...
        if isinstance(original_row_type, TypedDictType):
            ...
            if original_row_type.partial_fallback.type.has_base("typing.NamedTuple"):
                ...
                annotation_fields = {
                    name: AnyType(TypeOfAny.from_omitted_generics)
                    for name in expression_types
                }
                row_type = helpers.extend_oneoff_named_tuple(
        ...
        elif isinstance(original_row_type, Instance) and helpers.is_model_type(
            original_row_type.type
        ):
...
def merge_annotations_from_custom_method(
    ctx: MethodContext, django_context: DjangoContext
) -> MypyType:
    """
    ...
    for field_lookup in field_lookups:
        ...
        if field_lookup_type is None:
            if django_model.is_annotated:
                ...
                return reparametrize_queryset(
                    default_return_type,
                    [django_model.typ, AnyType(TypeOfAny.from_error)],
                )
        else:
...
def _infer_prefetch_queryset_type(
    queryset_expr: Expression, api: TypeChecker
) -> Instance | None:
...
def _resolve_prefetch_queryset_argument(
    ...
    if isinstance(queryset_type, Instance):
        ...
        if (
            elem_model is not None
            and elem_model.type.fullname != fullnames.MODEL_CLASS_FULLNAME
        ):
...
def _specialize_string_arg_to_literal(
    ...
    if arg_expr := helpers.get_call_argument_by_name(ctx, arg_name):
        if arg_value := helpers.resolve_string_attribute_value(
            arg_expr, django_context
        ):
            api = helpers.get_typechecker_api(ctx)
            return LiteralType(
                value=arg_value, fallback=api.named_generic_type("builtins.str", [])
            )

    return fallback_type


def specialize_prefetch_type(
    ...
    lookup_type = _specialize_string_arg_to_literal(
        ctx, django_context, "lookup", default.args[0]
    )
    to_attr_type = _specialize_string_arg_to_literal(
        ctx, django_context, "to_attr", default.args[2]
    )
...
def gather_flat_args(ctx: MethodContext) -> list[tuple[Expression | None, ProperType]]:
    ...
    arg_start_idx = 0
    for expr, typ, kind in zip(
        ctx.args[0], ctx.arg_types[0], ctx.arg_kinds[0], strict=False
    ):
...
def _get_selected_fields_from_queryset_type(qs_type: Instance) -> set[str] | None:
    ...
    if len(qs_type.args) > 1:
        ...
        if isinstance(row_type, TupleType):
            if row_type.partial_fallback.type.has_base("typing.NamedTuple"):
                return {
                    name
                    for name, sym in row_type.partial_fallback.type.names.items()
    ...
    if (
        qs_type.extra_attrs
        and qs_type.extra_attrs.immutable
        and isinstance(qs_type.extra_attrs.immutable, set)
    ):
    ...
    *,
    ...
    if isinstance(ctx.type, Instance):
        ...
        if selected_fields is not None:
            model_field_names = {
                f.name for f in django_context.get_model_fields(model.cls)
            }
...
def check_valid_prefetch_related_lookup(
    ...
    *,
    ...
    contenttypes_installed = django_context.apps_registry.is_installed(
    ...
    for through_attr in lookup.split(LOOKUP_SEP):
        ...
        if contenttypes_installed and is_generic_prefetch:
            ...
            current_model_cls = (
                rel_obj_descriptor.rel.related_model
                if rel_obj_descriptor.reverse
                else rel_obj_descriptor.rel.model
            )
...
def check_conflicting_lookups(
    ...
    is_conflicting_lookup = bool(
        observed_attr in qs_types and qs_types[observed_attr] != queryset_type
    )
...
def extract_prefetch_related_annotations(
    ctx: MethodContext, django_context: DjangoContext
) -> MypyType:
    """
    ...
    if not (
        ...
        and isinstance(
            (default_return_type := get_proper_type(ctx.default_return_type)), Instance
        )
        and (qs_model := helpers.get_model_info_from_qs_ctx(ctx, django_context))
        is not None
        and ctx.args
        and ctx.arg_types
        and ctx.arg_types[0]
    ...
    new_attrs: dict[
        str, MypyType
    ] = {}  # A mapping of field_name / types to add to the model
    qs_types: dict[
        str, Instance | None
    ] = {}  # A mapping of field_name / associated queryset type
    for expr, typ in gather_flat_args(ctx):
        if not (
            isinstance(typ, Instance)
            and typ.type.has_base(fullnames.PREFETCH_CLASS_FULLNAME)
        ):
            ...
            if lookup is not None:
                check_valid_prefetch_related_lookup(
                    ctx, lookup, qs_model, django_context
                )
        ...
        lookup = _resolve_prefetch_string_argument(
            typ.args[0], expr, django_context, "lookup"
        )

        # 2) Extract to_attr value from specialized type arg or call expression
        to_attr = _resolve_prefetch_string_argument(
            typ.args[2], expr, django_context, "to_attr"
        )
        ...
        if not to_attr and lookup:
            check_valid_prefetch_related_lookup(
                ...
                qs_model,
                django_context,
                is_generic_prefetch=typ.type.has_base(
                    fullnames.GENERIC_PREFETCH_CLASS_FULLNAME
                ),
            )
            check_conflicting_lookups(ctx, lookup, qs_types, queryset_type)
            qs_types[lookup] = queryset_type
    ...
    if len(default_return_type.args) > 1:
        ...
        if isinstance(original_row, Instance) and helpers.is_model_type(
            original_row.type
        ):
...
def _check_field_not_pk(
    ...
    *,
    ...
    if field in all_pk_fields:
        ...
        ctx.api.fail(
            f'"{method}()" does not support primary key fields{param_str}. Got "{field_name}"',
            ctx.context,
        )
        return False
    return True
...
def _extract_field_names_from_collection(
    ...
    if not (
        ...
        and isinstance(
            (collection_expr := ctx.args[arg_index][0]), (ListExpr, TupleExpr, SetExpr)
        )
    ):
        return None
...
def _extract_field_names_from_varargs(ctx: MethodContext) -> list[str]:
    return [
        lookup_value
        for lookup_type in (
            ctx.arg_types[0] if ctx.arg_types and ctx.arg_types[0] else []
        )
        if (lookup_value := helpers.get_literal_str_type(get_proper_type(lookup_type)))
        is not None
    ]
...
def _validate_select_related_lookup(
    ...
    for i, part in enumerate(lookup_parts):
        ...
        if i < len(lookup_parts) - 1:  # Not the last part
            try:
                field, observed_model = django_context.resolve_lookup_into_field(
                    observed_model, part
                )
                if field is None:
                    return False
...
def validate_select_related(
    ...
    for lookup_value in _extract_field_names_from_varargs(ctx):
        _validate_select_related_lookup(
            ctx, django_context, django_model.cls, lookup_value
        )
...
def _validate_bulk_update_field(
    ...
    *,
    ...
    return (
        ...
        and _check_field_not_pk(
            ctx, model_cls, field, field_name, method, attr_name=attr_name
        )
    )
...
def validate_bulk_update(
    ctx: MethodContext,
    django_context: DjangoContext,
    method: Literal["bulk_update", "abulk_update"],
) -> MypyType:
    """
    ...
    if (
        ...
        field_names := _extract_field_names_from_collection(
            ctx, django_context, arg_index=1
        )
    ) is None:
        return ctx.default_return_type
    ...
    if not field_names:
        ...
        if (
            isinstance(collection_expr, (ListExpr, TupleExpr, SetExpr))
            and not collection_expr.items
        ):
...
def _validate_bulk_create_unique_field(
    ...
    return (
        field := _try_get_field(ctx, model_cls, field_name, resolve_pk=True)
...
def validate_bulk_create(
    ...
    update_field_names = _extract_field_names_from_collection(
        ctx, django_context, arg_index=4
    )
    if update_field_names is not None:
        for field_name in update_field_names:
            _validate_bulk_update_field(
                ctx, django_model.cls, field_name, method, attr_name="update_fields"
            )

    unique_field_names = _extract_field_names_from_collection(
        ctx, django_context, arg_index=5
    )
    if unique_field_names is not None:
        for field_name in unique_field_names:
            _validate_bulk_create_unique_field(
                ctx, django_model.cls, field_name, method
            )
...
def _validate_order_by_lookup(
    ctx: MethodContext, model_cls: type[Model], parts: list[str]
) -> None:
    ...
    try:
        _, final_field, _, remainder = Query(model_cls).names_to_path(
            parts, model_cls._meta
        )
    except FieldError as exc:
        ctx.api.fail(exc.args[0], ctx.context)
        return
...
def validate_order_by(ctx: MethodContext, django_context: DjangoContext) -> MypyType:
    ...
    selected_fields = (
        _get_selected_fields_from_queryset_type(ctx.type)
        if isinstance(ctx.type, Instance)
        else None
    )
    annotated_fields = (
        _get_annotated_fields_from_queryset_type(ctx.type)
        if isinstance(ctx.type, Instance)
        else set()
    )
...
def _validate_defer_only_fields(
    ...
    *,
    ...
    query.add_deferred_loading(
        field_names
    ) if is_defer else query.add_immediate_loading(field_names)
...
def validate_defer_only(
    ctx: MethodContext, django_context: DjangoContext, *, is_defer: bool
) -> MypyType:
    if (
        django_model := helpers.get_model_info_from_qs_ctx(ctx, django_context)
    ) is None or not (field_names := _extract_field_names_from_varargs(ctx)):
        return ctx.default_return_type
