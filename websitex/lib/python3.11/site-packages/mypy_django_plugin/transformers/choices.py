...
def _is_lazy_string(ty: MypyType | None) -> bool:
    ...
    return isinstance(proper_type, Instance) and proper_type.type.has_base(
        fullnames.STR_PROMISE_FULLNAME
    )
...
def _try_replace_label(typ: ProperType, label_laziness: _LabelLaziness) -> MypyType:
    ...
    items = [
        ...
        and t.type.has_base(fullnames.STR_PROMISE_FULLNAME)
        == (label_laziness is _LabelLaziness.LAZY)
    ]
...
def _try_replace_value(
    typ: ProperType, base_type: Instance | None, has_empty_label: bool
) -> MypyType:
    """
    Attempt to replace a label with a modified version.
...
def transform_into_proper_attr_type(ctx: AttributeContext) -> MypyType:
    ...
    if node is None:
        ...
        if isinstance(_node_type, UnionType):
            ...
            _node_type = get_proper_type(
                _get_enum_type_from_union_of_literals(_node_type)
            )
    ...
    if (
        ...
        if isinstance(choice_arg, TupleType) and choice_arg.length() == 2:
            ...
            new_value_arg = _try_replace_value(
                value_arg, base_type, empty_label is not None
            )
            if new_label_arg is not label_arg or new_value_arg is not value_arg:
                new_choice_arg = choice_arg.copy_modified(
                    items=[new_value_arg, new_label_arg]
                )
        ...
        return _try_replace_label(
            default_attr_type, _LabelLaziness.make(_is_lazy_string(empty_label.type))
        )
