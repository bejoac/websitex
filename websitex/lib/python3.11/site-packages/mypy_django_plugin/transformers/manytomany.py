...
def get_related_manager_and_model(
    ctx: MethodContext,
) -> tuple[Instance, Instance, Instance] | None:
    """
    ...
    if (
        ...
        if (
            ...
            and isinstance(
                (_To := get_proper_type(many_related_manager.args[0])), Instance
            )
            and helpers.is_model_type(_To.type)
            and isinstance(
                (_Through := get_proper_type(many_related_manager.args[1])), Instance
            )
