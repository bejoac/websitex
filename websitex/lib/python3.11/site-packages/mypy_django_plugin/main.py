...
class NewSemanalDjangoPlugin(Plugin):
    ...
    def get_function_hook(
        ...
        if info:
            ...
            if helpers.is_model_type(info):
                return partial(
                    init_create.typecheck_model_init, django_context=self.django_context
                )
            ...
            if info.has_base(fullnames.PREFETCH_CLASS_FULLNAME):
                return partial(
                    querysets.specialize_prefetch_type,
                    django_context=self.django_context,
                )
    ...
    def manager_and_queryset_method_hooks(
        ...
        return {
            ...
            "create": partial(
                init_create.typecheck_model_create, django_context=self.django_context
            ),
            "acreate": partial(
                init_create.typecheck_model_acreate, django_context=self.django_context
            ),
            ...
            "prefetch_related": partial(
                querysets.extract_prefetch_related_annotations,
                django_context=self.django_context,
            ),
            "select_related": partial(
                querysets.validate_select_related, django_context=self.django_context
            ),
            "bulk_update": partial(
                querysets.validate_bulk_update,
                django_context=self.django_context,
                method="bulk_update",
            ),
            ...
            "bulk_create": partial(
                querysets.validate_bulk_create,
                django_context=self.django_context,
                method="bulk_create",
            ),
            ...
            "order_by": partial(
                querysets.validate_order_by, django_context=self.django_context
            ),
            "earliest": partial(
                querysets.validate_order_by, django_context=self.django_context
            ),
    ...
    def get_method_hook(
        ...
        if class_fullname.endswith("QueryDict") and info.has_base(
            fullnames.QUERYDICT_CLASS_FULLNAME
        ):
            return check_querydict_is_mutable

        if (
            method_name in self.manager_and_queryset_method_hooks
            and helpers.has_any_of_bases(
                info,
        ...
        if method_name == "get_field" and info.has_base(
        ...
        if helpers.has_any_of_bases(
            ...
            return partial(
                querysets.merge_annotations_from_custom_method,
                django_context=self.django_context,
            )
    ...
    def get_metaclass_hook(
        ...
        if fullname == fullnames.MODEL_METACLASS_FULLNAME:
            return partial(
                MetaclassAdjustments.adjust_model_class,
    ...
    def get_attribute_hook(
        ...
        if (
            info.has_base(fullnames.PERMISSION_MIXIN_CLASS_FULLNAME)
        ...
        if info.has_base(fullnames.ABSTRACT_USER_MODEL_FULLNAME) and attr_name in (
            "is_staff",
            "is_active",
        ):
            return partial(
        ...
        if (
            ...
            info.has_base(fullnames.CHOICES_CLASS_FULLNAME)
            and attr_name in {"label", "value"}
        ):
    ...
    def report_config_data(self, ctx: ReportConfigContext) -> dict[str, Any]:
        ...
        try:
            extra_data["django_stubs_ext_version"] = importlib.metadata.version(
                "django-stubs-ext"
            )
