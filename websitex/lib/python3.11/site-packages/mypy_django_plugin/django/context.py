...
class DjangoContext:
        ...
        *,
        ...
        if isinstance(field, Field):
            get_type = _get_field_get_type_from_model_type_info(
                model_info, getattr(field, "attname", field.name)
            )
    ...
    def solve_lookup_type(
        ...
        related_model = self.get_field_related_model_cls(field)
        sub_query = Query(related_model).solve_lookup_type(
            LOOKUP_SEP.join(query_parts[1:])
        )
    ...
    def _resolve_lookup_type_from_lookup_class(
        ...
        ctx: MethodContext,
        lookup_cls: type,
        field: Field[Any, Any] | ForeignObjectRel | None = None,
    ) -> MypyType | None:
        ...
        for lookup_base in helpers.iter_bases(lookup_info):
            if lookup_base.args and isinstance(
                (lookup_type := get_proper_type(lookup_base.args[0])), Instance
            ):
                ...
                if lookup_type.type.fullname == fullnames.FIELD_FULLNAME:
                    ...
                    return get_proper_type(
                        helpers.get_private_descriptor_type(
    ...
    def _resolve_annotated_field_lookup(
        ...
        """
        if (
            not helpers.is_annotated_model(model_instance.type)
            or not model_instance.extra_attrs
        ):
            return None
        ...
        if annotation_lookup_parts:
            ...
            if lookup_cls is not None:
                lookup_type = self._resolve_lookup_type_from_lookup_class(
                    ctx, lookup_cls
                )
    ...
    def resolve_lookup_expected_type(
        ...
        try:
            ...
            annotation_lookup = self._resolve_annotated_field_lookup(
                ctx, lookup, model_instance
            )
        ...
        resolved_type = self._resolve_lookup_type_from_lookup_class(
            ctx, lookup_cls, field
        )
