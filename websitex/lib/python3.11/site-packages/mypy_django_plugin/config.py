...
class DjangoPluginConfig:
    ...
    def parse_toml_file(self, filepath: Path) -> None:
        ...
        self.strict_model_abstract_attrs = config.get(
    ...
    def parse_ini_file(self, filepath: Path) -> None:
        ...
        try:
            self.strict_model_abstract_attrs = parser.getboolean(
                section, "strict_model_abstract_attrs", fallback=True
            )
        except ValueError:
            exit_with_error(
                INVALID_BOOL_SETTING.format(key="strict_model_abstract_attrs")
            )
